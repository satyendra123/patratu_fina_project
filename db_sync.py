...
def run_sync_once(connection):
    ...
    try:
        ...
        if TARGET_ISDELETED_COLUMN_NAME.lower() not in available_columns:
            raise RuntimeError(
                f"ISDELETED column [{TARGET_ISDELETED_COLUMN_NAME}] not found in {target_view}"
            )
        if TARGET_DEL_DATE_COLUMN_NAME.lower() not in available_columns:
            raise RuntimeError(
                f"DEL_DATE column [{TARGET_DEL_DATE_COLUMN_NAME}] not found in {target_view}"
            )

        cursor.execute(build_delete_sql(target_view))
        marked_deleted_rows = cursor.fetchone()[0]
...
def main():
    ...
    while True:
        ...
        try:
            ...
            elif not upd_date_sync_enabled:
                log(
                    f"UPD_DATE column [{TARGET_UPD_DATE_COLUMN_NAME}] not found in {target_view} | "
                    "UPD_DATE sync skipped"
                )
            log(
                f"Sync completed | target_view={target_view} | "
                f"marked_deleted_rows={marked_deleted_rows} | updated_verify_rows={updated_rows}"
            )
        except KeyboardInterrupt:
            log("Sync stopped by user")
            raise
